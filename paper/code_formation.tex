...
This framework raises a deep epistemological issue: if high-dimensional field dynamics are the substrate from which discrete codes crystallize, then \textit{measurement itself is dimensional collapse}. When we record from electrodes, we are not observing the high-dimensional field---we are observing the output of a code formation process. The electrode is another bottleneck.

This may explain why neuroscience has historically focused on spikes and discrete codes: they are what survives measurement. The continuous, high-dimensional field dynamics---if they carry the ``interesting'' computation---are precisely what our instruments cannot access without collapsing.
