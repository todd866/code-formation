...
Loss function & Effective potential & Lyapunov-like objective \\
...
For \textit{evaluation only}, we partition the ring into 6 equal sectors and ask: do the learned codes preserve this structure? We treat these sectors as ``semantic classes'' and measure the Adjusted Rand Index (ARI) between k-means clusters of learned codes and ground-truth sectors. (The choice of $n=6$ is for convenience; results are qualitatively similar for 4--8 sectors.)
...
\caption{\textbf{What gets preserved.} Top: Input effectively lives in a 2-D linear subspace (PC1--2 capture the ring; higher PCs are embedding noise). The generative manifold is $S^1$ (1-D), but it requires 2 linear dimensions to embed without self-intersection. Bottom: Encoder learns to preserve these high-variance directions. At $k=2$ with noise, discretization occurs.}
\label{fig:pca}
\end{figure}
...
\caption{\textbf{SR-like behavior and expectation bias.} (a) Learned codes (colored centroids) and ambiguous test signals ($\times$, positioned near decision boundaries). (b) Decoding accuracy peaks at intermediate noise---the hallmark SR curve. The star marks the SR peak for the unbiased condition. (c) Effective dimensionality (participation ratio) increases monotonically with noise, reflecting expansion into the ambient 512-D space. (d) Expectation-biased SR: a high-dimensional expectation signal added in 512-D input space (before the bottleneck) biases which basin ambiguous inputs land in. Expectation effectively tilts the potential landscape, widening the basin of attraction for the expected code.}
\label{fig:sr}
\end{figure}

This reframes SR in geometric terms: the ambiguous signal is trapped on a low-dimensional saddle (the decision boundary). Noise does not merely add energy for barrier-crossing; it restores rank to the covariance matrix, allowing system trajectories to utilize the full dimensionality of the ambient space ($N=512$) to bypass the bottleneck-induced constriction ($k=2$). Optimal performance occurs when noise is sufficient to escape boundaries but not so large as to cause random wandering.

\subsection{Expectation-Biased Stochastic Resonance}

The SR phenomenon described above assumes \textit{isotropic} noise---random fluctuations with no preferred direction. But in systems with top-down predictions, the noise need not be isotropic. We propose \textbf{expectation-biased stochastic resonance} (EBSR): the same SR dynamics, but with noise biased toward an expected code.

This maps naturally onto drift-diffusion dynamics. In Langevin form:
\begin{equation}
    d\mathbf{x} = -\nabla U(\mathbf{x})\,dt + \mathbf{F}_{\text{bias}}\,dt + \sigma\,d\mathbf{W}
\end{equation}
where $U(\mathbf{x})$ is the effective potential (learned by the decoder), $\mathbf{F}_{\text{bias}}$ is the expectation-induced drift toward a predicted code, and $\sigma\,d\mathbf{W}$ is the stochastic noise term. The expectation term breaks the symmetry of the potential landscape, effectively tilting the basin of attraction so that noise preferentially kicks the state into the expected code.

In discrete form for our simulation:
\begin{equation}
    \mathbf{z}_{t+1} = \mathbf{z}_t + \eta\boldsymbol{\xi}_t + \beta\mathbf{b}
\end{equation}
where $\boldsymbol{\xi}_t \sim \mathcal{N}(0, I)$ is isotropic noise (scaled by $\eta$), $\mathbf{b}$ is a unit vector pointing toward the expected code centroid, and $\beta$ controls the strength of expectation bias.

Figure~\ref{fig:sr}(d) demonstrates this in our simulation. Crucially, we add expectation not in the low-dimensional code space, but in the original 512-dimensional input space---before the bottleneck. This models the key biological intuition: top-down signals (e.g., from higher cortical areas) enter the system at the high-dimensional stage and bias which code emerges after dimensional collapse. The expectation effectively widens the basin of attraction for the target code while narrowing it for competitors.

The results show that expectation reliably biases which basin ambiguous inputs land in. When the expectation matches the true category, accuracy improves; when it mismatches, systematic errors occur.

\paragraph{The evolutionary argument.} Stochastic resonance is widespread in biological sensory systems---it appears to be a ``free'' consequence of noisy, threshold-based neural circuits \citep{gammaitoni1998}. Similarly, top-down predictions are ubiquitous in cortex. Given that both mechanisms exist independently, \textit{not} combining them would leave performance on the table. We therefore suggest that EBSR is not merely possible but likely: any system that already exploits SR for signal detection, and that has access to top-down expectations, would naturally evolve to bias its noise toward predicted outcomes.

This yields a three-ingredient model of biological code formation:
\begin{center}
\textit{Bottleneck} + \textit{Noise} + \textit{Predictions} $\to$ \textit{Expectation-biased discrete codes}
\end{center}
...
\subsection{Beta-Gamma Coupling and Ephaptic Fields}

A speculative but intriguing application concerns the interface between oscillatory field dynamics and neural spiking. Consider the beta-gamma coupling observed in cortex \citep{lundqvist2016gamma,miller2018working}: beta oscillations ($\sim$15--30 Hz) modulate the amplitude of nested gamma oscillations ($\sim$30--100 Hz), which in turn coordinate spike timing. Extracellular fields generated by neural activity can, in turn, influence nearby neurons through ephaptic coupling \citep{anastassiou2011ephaptic}.

A common interpretation treats beta as ``simple'' (low-dimensional) because it is slow. We suggest an alternative: low-frequency oscillations can support \textit{more} spatial modes (higher effective dimensionality) precisely because they vary slowly enough for complex spatial patterns to be sustained. Gamma, being faster, acts as a bandwidth-limited channel---a bottleneck that compresses the high-dimensional beta/field structure into a lower-dimensional but higher-information-density code. Spikes then provide a further discretization step.

In this view, the cascade is:
\begin{center}
\textit{Beta field (high-D)} $\to$ \textit{Gamma (low-D, high info)} $\to$ \textit{Spikes (discrete codes)}
\end{center}

Each arrow represents a noisy, capacity-limited interface of the kind simulated here. The ``code formation'' we demonstrate---discretization via bottleneck + noise---may thus occur at multiple stages: first at the beta-to-gamma transition (field compression), and again at the gamma-to-spike transition (neural discretization).

Crucially, this interpretation connects directly to EBSR: if beta oscillations carry top-down predictions as high-dimensional spatial patterns, then beta-gamma coupling \textit{is} expectation-biased stochastic resonance. The beta field provides the ``expectation'' signal that enters the system before the gamma bottleneck, biasing which discrete code emerges. In this picture, the brain is performing SR on itself---using slow oscillations to shape the noise statistics of fast oscillations, thereby implementing prediction-guided code formation.

This inverts the standard interpretation and suggests that slow oscillations are not simple carriers but high-dimensional prediction signals from which discrete neural codes crystallize.

\subsection{The Measurement Problem}

This framework raises a deep epistemological issue: if high-dimensional field dynamics are the substrate from which discrete codes crystallize, then \textit{measurement itself is dimensional collapse} \citep{todd2025falsifiability}. When we record from electrodes, we are not observing the high-dimensional field---we are observing the output of a code formation process. The electrode is another bottleneck.

This may explain why neuroscience has historically focused on spikes and discrete codes: they are what survives measurement. The continuous, high-dimensional field dynamics---if they carry the ``interesting'' computation---are precisely what our instruments cannot access without collapsing. Claims about such high-dimensional substrates may be fundamentally difficult to falsify using measurements that necessarily discretize.

The problem generalizes beyond neuroscience. In any system where high-dimensional energy flows into low-dimensional codes---protein folding, gene regulation, immune recognition, ecological dynamics---we face the same issue. We measure the codes (the outputs), not the high-dimensional processes that formed them. The simulation presented here operates entirely in the measurable regime (we have full access to the 512-D input and the $k$-D codes), but it may serve as a window into dynamics that, in physical systems, occur ``upstream'' of observation.
...
However, a thermodynamic reinterpretation is natural: treat the reconstruction loss as an effective potential $U(\mathbf{c})$ and the channel noise variance as proportional to temperature ($\sigma^2 \propto k_B T$). The system then minimizes an effective free energy $F = \langle U \rangle - TS$, where $S$ is the entropy of the code distribution. Discretization would correspond to the formation of a small number of deep basins in the effective potential, each with a finite catchment volume in code space. At fixed channel capacity, increasing temperature (noise) favours coarse, robust basins over fine-grained structure.

In this picture, the stochastic resonance phenomenon becomes standard barrier-crossing dynamics: noise provides the thermal energy to escape local minima and explore the potential landscape. The ``information'' emerges when the system settles into a basin---the uncertainty collapses, and the basin identity carries $\log_2 n$ bits. This collapse is a natural candidate for the ``projection'' event where thermodynamic costs are paid \citep{todd2025maxwell}.

A key insight from recent work on biological thermodynamics is that continuous high-dimensional substrates can defer the Landauer cost of bit erasure until dimensional projection occurs \citep{todd2025maxwell}. In our simulation, the encoder-decoder system operates analogously: information exists as continuous correlations in the high-D input, and the thermodynamic ``payment'' would occur at the bottleneck where dimensional collapse forces discretization. This suggests that the code formation process we demonstrate may be energetically favored precisely because it defers projection until capacity constraints make it inevitable.

We have not developed this thermodynamic model here; doing so properly would require specifying the potential landscape, computing Kramers rates for basin transitions, and tracking entropy production along trajectories. This is a natural direction for future work, connecting to stochastic thermodynamics and the physics of biological information processing.
