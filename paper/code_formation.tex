...
Loss function & Energy & Hamiltonian to minimize \\
...
One might ask how this relates to Landauer's principle, which bounds the energy cost of bit erasure \citep{landauer1961, bennett1982}. Our simulation has no explicit energy model, so we cannot make direct thermodynamic claims. However, we note that the discretization here arises from the \textit{interaction} of compression and noise, without explicit erasure operations. A physical implementation would still obey Landauer, but this suggests that some ``symbol-like'' processing may occur in regimes quite different from traditional digital logic. We flag this as an open question rather than a claim.
...
The simulation is implemented in Python using PyTorch. Code and figures are available at \texttt{github.com/todd866/code-formation}. Key parameters:
\begin{itemize}
