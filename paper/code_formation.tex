...
\caption{\textbf{SR-like behavior and expectation bias.} (a) Learned codes and ambiguous test signals on basin boundaries. (b) Accuracy peaks at intermediate noise---the hallmark SR curve. (c) Effective dimensionality (PR) increases monotonically with noise. (d) Expectation-biased SR: when a high-dimensional ``expectation'' signal is added before the bottleneck, it biases which basin ambiguous inputs land in.}
\label{fig:sr}
\end{figure}
...
Formally, let $\mathbf{z}_0$ be an ambiguous code sitting on a decision boundary. Its trajectory under noisy dynamics becomes:
\begin{equation}
